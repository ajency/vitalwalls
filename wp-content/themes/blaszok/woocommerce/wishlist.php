...
<form id="yith-wcwl-form" action="<?php echo esc_url( $yith_wcwl->get_wishlist_url() ) ?>" method="post">
	...
	<table class="shop_table cart wishlist_table" cellspacing="0">
		...
		<tr>
			...
			<th class="product-name"><span class="nobr"><?php _e( 'Product', 'mpcth' ); ?></span></th>
			...
			foreach( $wishlist as $values ) :
				...
				if( $product_obj !== false && $product_obj->exists() ) : ?>
					<tr id="yith-wcwl-row-<?php echo $values['ID'] ?>">
						<td class="product-remove"><a href="javascript:void(0)" onclick="remove_item_from_wishlist( '<?php echo esc_url( $yith_wcwl->get_remove_url( $values['ID'] ) )?>', 'yith-wcwl-row-<?php echo $values['ID'] ?>');" class="remove mpcth-color-main-color" title="<?php _e( 'Remove this product', 'yit' ) ?>">&times;</a></td>
	...
	<div class="shop_table cart wishlist_table mpcth-mobile-wishlist">
		<div>
		<?php
		if( count( $wishlist ) > 0 ) :
			foreach( $wishlist as $values ) :
				...
				if( $product_obj !== false && $product_obj->exists() ) : ?>
					<div id="yith-wcwl-row-<?php echo $values['ID'] ?>" class="mpcth-wishlist-item">
						<div class="product-name">
							<a href="<?php echo esc_url( get_permalink( apply_filters( 'woocommerce_in_cart_product', $values['prod_id'] ) ) ) ?>"><?php echo apply_filters( 'woocommerce_in_cartproduct_obj_title', $product_obj->get_title(), $product_obj ) ?></a>
						</div>
						<div class="product-remove"><a href="javascript:void(0)" onclick="remove_item_from_wishlist( '<?php echo esc_url( $yith_wcwl->get_remove_url( $values['ID'] ) )?>', 'yith-wcwl-row-<?php echo $values['ID'] ?>');" class="remove mpcth-color-main-color" title="<?php _e( 'Remove this product', 'yit' ) ?>">&times;</a></div>
						...
						<?php if( get_option( 'yith_wcwl_price_show' ) == 'yes' ) : ?>
							<div class="product-price">
								<div class="product-price"><span class="nobr"><?php _e( 'Unit Price', 'yit' ) ?></span></div>
								<?php
								if( $product_obj->price != '0' ) {
									$wc_price = function_exists('wc_price') ? 'wc_price' : 'woocommerce_price';
						...
						<?php if( get_option( 'yith_wcwl_stock_show' ) == 'yes' ) : ?>
							<div class="product-stock-status">
								<div class="product-stock-status"><span class="nobr"><?php _e( 'Stock Status', 'yit' ) ?></span></div>
								<?php
								$availability = $product_obj->get_availability();
								$stock_status = $availability['class'];
