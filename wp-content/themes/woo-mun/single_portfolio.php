...
<div class="mainwrap">
					...
					<div class = "imageFrame"></div>
						...
						if(!$portfolio['show_video'][0]){
							...
							<?php } else { 
								...
								$video = preg_replace('/height=\"(\d)*\"/', 'height="6500"', $video);	
								echo $video;
							} ?>						
						</div>	
					...
					<p>
						<?php if($portfolio['detail_active'][0]) {
							if($portfolio['detail_active'][0]) { ?>
							  <i class="fa  icon-link"></i><?php if (!function_exists('icl_object_id') or (ICL_LANGUAGE_CODE == $sitepress->get_default_language()) ) { echo pmc_stripText($pmc_data['port_project_url']); } else {  _e('Project URL:','wp-munditia'); } ?> <span class="link"><a target="_blank" href="http://<?php echo $portfolio['website_url'][0] ?>" title="project url"><?php echo $portfolio['website_url'][0] ?></a></span>  </br>
						<?php } else { ?>
							   <i class="fa  icon-link"></i><?php if (!function_exists('icl_object_id') or (ICL_LANGUAGE_CODE == $sitepress->get_default_language()) ) { echo pmc_stripText($pmc_data['port_project_url']); } else {  _e('Project URL:','wp-munditia'); } ?> <span class="link"><a title="project url"><?php echo $portfolio['website_url'][0] ?></a></span> 
						<?php }  ?>	
						<?php } ?>
						<?php if($portfolio['author'][0] !='') {?>
							<i class="fa fa-user"></i><?php if (!function_exists('icl_object_id') or (ICL_LANGUAGE_CODE == $sitepress->get_default_language()) ) { echo pmc_stripText($pmc_data['port_project_designer']); } else {  _e('Project designer:','wp-munditia'); } ?> <span class="authorp port"><?php echo $portfolio['author'][0] ?></span><br>
						<?php } ?>
						<?php if($portfolio['date'][0] !='') {?>
							<i class="fa fa-calendar"></i><?php if (!function_exists('icl_object_id') or (ICL_LANGUAGE_CODE == $sitepress->get_default_language()) ) { echo pmc_stripText($pmc_data['port_project_date']); } else {  _e('Date of completion:','wp-munditia'); } ?> <span class="posted-date port"><?php echo $portfolio['date'][0] ?></span><br>
						<?php } ?>
						<?php if($portfolio['customer'][0] !='') {?>
							<i class="fa fa-user"></i><?php if (!function_exists('icl_object_id') or (ICL_LANGUAGE_CODE == $sitepress->get_default_language()) ) { echo pmc_stripText($pmc_data['port_project_client']); } else {  _e('Client:','wp-munditia'); } ?> <span class="author port"><?php echo $portfolio['customer'][0] ?></span><br>
						<?php } ?>								
					</p>
	...
	<?php endif; ?>
		<div class="portfolio">		
			...
			<h3 class="h3border"><?php echo pmc_translation('port_project_related', 'Related <span>project</span>'); ?></h3>
			<div class="titleborder"></div>		
			<div id="portitems4">
				<?php pmc_portfolio('port3',3,'port',3,'',get_the_id()) ?>	
