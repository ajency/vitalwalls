						...
						<td class="product-quantity">
							<?php
								if ( $_product->is_sold_individually() ) {
									...
									$pmc_data_min = apply_filters( 'woocommerce_cart_item_data_min', '', $_product );
									$pmc_data_max = ( $_product->backorders_allowed() ) ? '' : $_product->get_stock_quantity();
									$pmc_data_max = apply_filters( 'woocommerce_cart_item_data_max', $pmc_data_max, $_product );

									$product_quantity = sprintf( '<div class="quantity"><input name="cart[%s][qty]" data-min="%s" data-max="%s" value="%s" size="4" title="Qty" class="input-text qty text" maxlength="12" /></div>', $cart_item_key, $pmc_data_min, $pmc_data_max, esc_attr( $values['quantity'] ) );
								}
