            ...
            jQuery(document).ready(function(){
               ...
               /* jQuery('select[name=attribute_size] option:eq(1)').attr('selected', 'selected');
    			...
    			jQuery(document).bind('oB_init', function() {
                    ...
                    //Get price from main page
					var price = jQuery(".single_variation .price ins .amount").text();
					var sizepopupVal = jQuery("#size_popup").val();
					if(sizepopupVal!="")
                     {
						jQuery("#amount_popup").text(price);
						alert ("If amt popup"+jQuery("#amount_popup").text());
					 }
					else
					 {
						jQuery("#amount_popup").text("");
						alert ("Else amt popup"+jQuery("#amount_popup").text());
					 }
...
/*function register_ajax_scripts() {
	...
	wp_enqueue_script( 'script_AjaxFetchPrice', get_template_directory_uri().'/ajaxFetchPrice.js', 'jquery', true);
	wp_enqueue_script( 'script_AjaxFetchPrice' );
	...
	wp_localize_script( 'script_AjaxFetchPrice', 'ajaxFetchPrice_script', array( 'ajaxurl' => admin_url( 'admin-ajax.php' ) ) );
}
 
add_action( 'enqueue_scripts', 'register_ajax_scripts' );
...
/*function get_product_variation_price($variation_id) {
    ...
    echo $product->product_custom_fields['_price'][0];
	die(); // This is required for a proper response.
}
 
//add_action( 'wp_ajax_script_handle', 'get_product_variation_price' ); // Hook for the WordPress Dashboard.
add_action( 'wp_ajax_script_handle_nopriv', 'get_product_variation_price' ); // Hook for the user facing side of the site. */
