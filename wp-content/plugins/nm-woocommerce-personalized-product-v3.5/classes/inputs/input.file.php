...
class NM_File extends NM_Inputs_wooproduct{
	...
	private function get_settings(){
		...
		return array (
				...
				/* 'button_label_upload' => array (
						'type' => 'text',
						'title' => __ ( 'Button label (upload files)', 'nm-personalizedproduct' ),
						'desc' => __ ( 'Type button label e.g: Upload Photos', 'nm-personalizedproduct' )
				), */
				...
				'chunk_size' => array (
						'type' => 'text',
						'title' => __ ( 'chunk size', 'nm-personalizedproduct' ),
						'desc' => __ ( 'Enables you to chunk the large file into smaller pieces, e.g: 1mb', 'nm-personalizedproduct' )
				),
	...
	<script type="text/javascript">	
		...
		jQuery(function($){
			...
			uploader_<?php echo $args['id']?> = new plupload.Uploader({
				...
				multipart_params 	: {'action' : 'nm_personalizedproduct_upload_file'},
				...
				init: {
					...
					FileUploaded: function(up, file, info){
						...
						if(ext == 'png' || ext == 'gif' || ext == 'jpg' || ext == 'jpeg'){
							...
							file_thumb = nm_personalizedproduct_vars.file_upload_path_thumb + obj_resp.file_name;
							...
							if('<?php echo $args['photo-editing']; ?>' === 'on'){
								...
								$filelist_DIV.find('#u_i_c_' + file.id).find('.u_i_c_tools_edit').append('<a onclick="return launch_aviary_editor(\'thumb_'+file.id+'\', \''+file_full+'\', \''+obj_resp.file_name+'\', \''+editing_tools+'\')" href="javascript:;" title="Edit"><img width="15" src="'+nm_personalizedproduct_vars.plugin_url+'/images/edit.png" /></a>');
							}
	
							// zoom effect
							$filelist_DIV.find('#u_i_c_' + file.id).find('.u_i_c_tools_zoom').append('<a href="#TB_inline?width=<?php echo $popup_width?>&height=<?php echo $popup_height?>&inlineId=u_i_c_big'+file.id+'" class="thickbox" title="'+obj_resp.file_name+'"><img width="15" src="'+nm_personalizedproduct_vars.plugin_url+'/images/zoom.png" /></a>');
							is_image = true;
						}else{
