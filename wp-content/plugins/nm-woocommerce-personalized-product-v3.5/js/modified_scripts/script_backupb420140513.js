...
jQuery(function($){
	...
	$(".nm-productmeta-box").find('select,input:checkbox,input:radio').on('change', function(){
        ...
        alert("Change frame in main page");

		//console.log('im dynamic');
		if ($(".single_variation .price .amount").length > 0){
			var base_amount = $(".single_variation .price .amount").text(); //actual amount of the product
            alert("if base amount## "+base_amount);
            ...
            alert("else base amount "+base_amount);
		}
		...
		if(updated_price != 0){
            ...
            alert("total price "+total_price+"\n"+nm_personalizedproduct_vars.woo_currency+"\n"+total_price);
			...
			html += '<strong>' + nm_personalizedproduct_vars.option_amount_text + ':'+totalamt + '</strong><br>';
			html += '</div>';
